...
for line in $(plugin_read_list PUSH) ; do
  ...
  if [[ ${#tokens[@]} -gt 1 ]] ; then
    ...
    # if the service is prebuilt, tag the prebuilt image to match and push it
    if prebuilt_image=$(get_prebuilt_image "$service" "${built_images[@]}") ; then
      echo "~~~ :docker: Pushing prebuilt $prebuilt_image to $image" >&2;
      plugin_prompt_and_run docker tag "$prebuilt_image" "$image"
      plugin_prompt_and_run docker push "$image"
    fi
  fi
done
